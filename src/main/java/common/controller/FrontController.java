...
@WebServlet(urlPatterns = { "*.do" }, initParams = {
		@WebInitParam(name = "config", value = "D:\\multicampus\\java-workspace\\MyBatisWeb\\src\\main\\webapp\\WEB-INF\\command.properties") })
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig conf) throws ServletException {
		...
		try {
			...
			if (set != null) {
				...
				System.out.println("cmdMap 저장 완료 : cmdMap.size() => " + cmdMap.size());
			} // if----
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void process(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("instance===>" + instance); // 객체의 주솟값
